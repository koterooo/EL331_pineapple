# All part of this source code is generated by ChatGPT
...
def kwic(text, target_ngram, window=5):
    ...
    for idx, ng in enumerate(ngrams):
        if ng.lower() == target_ngram.lower():
            left_context = words[max(0, idx-5):idx]
            right_context = words[idx+n:idx+n+5]

            # Colorize the target
            colored_target = colored(ng, 'cyan', attrs=['bold'])

            # Display
            print(' '.join(left_context) + ' ' + colored_target + ' ' + ' '.join(right_context))
